...
def main():
    ...
    for idx in valid_indices:
        ...
        single_view = np.array([[90, 270, 0]], dtype=np.float32)
        ref_images_list = []
        for j_img, sample_3d in enumerate(trellis_objects):
            ...
            for i_img in range(n_images):
                ...
                for j_img in range(n_images):
                    if i_img == j_img:
                        ...
                        l2_coord_matches[i_img][j_img] = np.array([90, 270, 0])
                        lpips_coord_matches[i_img][j_img] = np.array([90, 270, 0])
                        clip_coord_matches[i_img][j_img] = np.array([90, 270, 0])
                        dino_coord_matches[i_img][j_img] = np.array([90, 270, 0])
                    ...
                    clip_idx = clip_losses.argmin()
                    clip_image_matches[i_img, j_img] = candidate_views[clip_idx]
                    clip_coord_matches[i_img][j_img] = camera_coords[clip_idx]
                    ...
                    dino_idx = dino_losses.argmin()
                    dino_image_matches[i_img, j_img] = candidate_views[dino_idx]
                    dino_coord_matches[i_img][j_img] = camera_coords[dino_idx]
        ...
        out_path = Path(f"data/Trellis_test/{dataset_name}/{trial_name}")
        ...
        def make_grid_image(matches_tensor):
            rows = []
            for row_i in range(n_images):
                row_tensors = []
                for col_j in range(n_images):
                    row_tensors.append(matches_tensor[row_i, col_j])
                row_cat = torch.cat(row_tensors, dim=2)
                rows.append(row_cat)
            full_grid = torch.cat(rows, dim=1)
            return full_grid.permute(1, 2, 0).cpu().numpy()
        ...
        print(f"Done trial => {trial_name}, results in {out_path}")
        n_processed += 1
